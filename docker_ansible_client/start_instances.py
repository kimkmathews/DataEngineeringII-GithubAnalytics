...
instance_prod = nova.servers.create(name="prod_server_with_docker_"+str(identifier), image=image, flavor=flavor, key_name='TeamKey',userdata=userdata_prod, nics=nics,security_groups=secgroups)
instance_dev = nova.servers.create(name="dev_server_"+str(identifier), image=image, flavor=flavor, key_name='TeamKey',userdata=userdata_dev, nics=nics,security_groups=secgroups)
...
#add ip to ansible host

file_content = '''[servers]
prodserver ansible_host={} 
devserver ansible_host={}
[all:vars]
ansible_python_interpreter=/usr/bin/python3
[prodserver]
prodserver ansible_connection=ssh ansible_user=appuser
[devserver]
devserver ansible_connection=ssh ansible_user=appuser'''.format(ip_address_prod, ip_address_dev)

# We automate the process by writing the file to the ansible host and switchin to root 
file_path = '/etc/ansible/hosts'

# Switch to root and write content to file
sudo_command = 'sudo bash -c "echo \'{}\' > {}"'.format(file_content, file_path)

# Execute the sudo_command to switch to root user and write the file
